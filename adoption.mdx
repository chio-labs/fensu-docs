...
depends on where you are starting. The first two situations below are where Fensu
pays off fastest; the third needs more judgment.

## New repositories

The defaults are calibrated for a fresh repo. Turn everything on from the first
commit: there is very little to fix, because there is little code yet, and the rules
keep the structure intact as it grows.
...
This is the strongest first experience. Whether the repo is tidy or a little
inconsistent, if it is small enough that reaching zero faults is realistic, adoption
is mostly a single pass that leaves it on a coherent architecture.
...
3. **Drive the faults to zero.** Hand the output to a coding agent and let it work
   ...
   Most faults are local, mechanical corrections. The few that would require a
   genuine structural decision are worth pausing on rather than rushing.
