...
Fensu ships a **coherent default architecture**, not a blank rule language. You
...
Concretely, the default lays out code as domains built from a small set of roles.
A domain holds the roles directly, or splits into named subdomains that do:
...
Because Fensu enforces the structure, it can also render it meaningfully.
...
modes, and [`fensu skills`](/cli/skills) closes the loop: it generates agent
...
Fensu turns that hard-won structure into a reusable default. The default is
...
<Note>
  The PyPI distribution is `fensu`; the command is `fensu`. Install with
  `pip install fensu` and run `fensu`. Fensu is functional and self-hosting,
  but pre-release.
</Note>
