---
title: "Fensu (フェンス)"
description: "Executable architecture rules for Python, Rust, TypeScript, and Svelte repositories."
---

<p className="text-right text-sm italic text-gray-500 dark:text-gray-400">
...
Fensu is an architecture linter for Python, Rust, TypeScript, and Svelte projects. It analyzes your source and
...
Python uses Fensu's core `FF` policy. Rust, TypeScript, SvelteKit, and framework-specific
Python projects use native rule packs selected per analyzer target. See
[Rust](/concepts/analyzers/rust),
[TypeScript and SvelteKit](/concepts/analyzers/typescript-sveltekit), and
[Native rule packs](/concepts/rule-packs).

Fensu ships a **coherent default architecture**, not a blank canvas. You
adopt it and get a sensible starting point, then disable rules, tune thresholds,
activate a framework-specific native policy, or add your own rules deliberately
once you know what you are doing.

The Python default lays out code as domains built from a small set of roles.
...
- [`fensu map`](/cli/map) helps you see that shape again, as a deterministic
  downstream or upstream call tree with clickable `path:line` locations.
...
## Beyond faults: duplicated code

Some drift breaks no rule. When several modules each grow their own copy of one
behaviour, every copy can sit in a valid place with a valid shape, and the copies
still drift apart as fixes land in only one of them.
[`fensu dupes`](/cli/dupes) reports those copies as ranked clusters, with the
lines where they disagree. It is advisory: it never fails a build, and its
findings never need to reach zero.

<Card title="Duplicated code" icon="clone" href="/concepts/duplicated-code">
  Why a copy is architectural drift, and how to diagnose and consolidate it.
</Card>
...
<CardGroup cols={2}>
  ...
  <Card title="Native rule packs" icon="boxes-stacked" href="/concepts/rule-packs">
