...
configuration. Both take rule-code **prefixes**: `SF` is every core rule, `SFR` a
family, `SFR3` a hundreds bucket, `SFR301` one rule; `X` is every custom rule,
