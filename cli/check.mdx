...
fensu check [paths...] [--warn] [--no-color]
             [--cache | --no-cache] [--cache-stats] [--target TARGET]
```
...
[scopes](/concepts/architecture-model#scopes), and applies each scope's rules. An
explicit multi-target configuration runs every target and produces one report.
Selected [repository rules](/concepts/repository-rules) then run exactly once over
that complete target registry.

## Arguments and options

| Argument | Description |
| --- | --- |
| `paths...` | Optional for Python and web targets. Use these paths instead of configured `roots`. Rust targets use Cargo discovery and reject positional paths. |
...
| `--target TARGET` | Evaluate only one named analyzer target; repository rules are skipped. |
...
disabled otherwise, and `--no-color` forces it off. With several configured
...
selects one Dagster-native rule; `FPRS` selects the complete Rust policy. To narrow a run, adjust those keys rather than the
command:
...
Pack selectors are available only after activation with `rule_packs`; see
