...
runs; a custom code that starts with `SF` is rejected at load, so a project can
...
Name modules after the policy they implement. SFR706 rejects filenames that are
exactly a core or custom rule code, such as `sft104.py` or `xac001.py`, because a
...
SFR707 requires each configured custom rule to have at least
