...
from fensu import Family, Fault, File, RuleContext, rule
...
def no_global_client(*, file: File, ctx: RuleContext) -> list[Fault]:
    return [
        ctx.fault(node=node)
...
```python
from fensu import rule, Fault, Family, Severity, Threshold, RuleContext, RuleOption
```
...
| `options` | no | Immutable typed options this rule declares and may read through `ctx.option()`. |
...
### Typed check subjects

Prefer a typed callback shape. A file rule runs once for each
...
- `file` is an immutable project-relative identity for the current source.
- `ctx` is a [`RuleContext`](#the-rule-context).
- The return value is a list of [`Fault`](#the-fault) objects.

A project rule runs exactly once for the target, including when discovery finds no
source files:

```python
from fensu import Project


def check(*, project: Project, ctx: RuleContext) -> list[Fault]: ...
```

Project rules have no elected source anchor. They inspect `ctx.project.tree` and
`ctx.graph`, and report against an explicit confined path with
`ctx.path_fault(path=...)` or `ctx.fault_at(location=...)`.

A repository rule runs once only during an aggregate all-target check and queries
explicit named target handles:

```python
from fensu import Repository


def check(*, repository: Repository, ctx: RuleContext) -> list[Fault]: ...
```

Its analyzers declare the target capabilities it may read. Repository policy lives
once in top-level `[repository_rules]`; `fensu check --target NAME` skips these
rules rather than providing a partial target registry. See
[Repository rules](/concepts/repository-rules) for configuration, target-local
facts, diagnostic translation, and cache behavior.

The legacy `(*, module: ast.Module, ctx: RuleContext)` callback remains supported
for Python rules that need unrestricted Python AST access. New capabilities arrive
through typed subjects and context rather than additional callback parameters.
...
includes current-file facts, a deterministic project tree, a resolved architecture
graph, source text, and backend-neutral syntax. Legacy Python callbacks may still
walk `module` when a rule needs unrestricted Python AST access.

```python
functions = ctx.facts.functions()                  # current-file semantic facts
position = ctx.project.tree.position(file.path)    # typed layout facts
imports = ctx.graph.imports(file)                  # resolved static imports
...
`ctx.project.tree` contains only the authoritative discovered inventory. Paths are
normalized project-relative POSIX `ProjectPath` values, and results are returned in
deterministic order. Its `paths`, `files`, `children()`, `descendants()`, `glob()`,
`files_under()`, and `position()` queries record the exact answer used by the active
rule.

For Python targets, `ctx.graph` exposes discovered module nodes, authored imports,
explicit resolved or unresolved edges, direct dependencies and dependents, and
deterministic static cycles. It resolves only imports that can be proven against
the discovered project; external, ambiguous, and dynamic imports are never guessed.

Every project-tree, graph, and `ctx.project` query records its input for cache
invalidation. The active requester is inferred for typed rules; legacy explicit
project queries may pass `requester=ctx.path`. Construct diagnostics through the
context as well:
...
Use `RuleOption` when one reusable rule needs repository-specific policy values.
The rule author owns the **default**; `fensu.toml` may provide an **override**; the
rule receives the resulting **current value**. Only overrides belong in TOML.

```python
_ALLOWED_NAMES = RuleOption.string_list(
    name="allowed_names",
    default=(),
    description="Names permitted by this policy.",
)
...
@rule(
    ...
    family=Family.CUSTOM,
    slug="approved-names",
    message="name is not approved",
    options=(_ALLOWED_NAMES,),
)
def approved_names(*, module: ast.Module, ctx: RuleContext) -> list[Fault]:
    allowed_names: tuple[str, ...] = ctx.option(_ALLOWED_NAMES)
    ...
```

Supported constructors are `boolean`, `integer`, `string`, `string_list`, and
`integer_list`. Lists resolve to immutable tuples. Integer declarations may set
`minimum` and `maximum`, strings may set `choices`, and lists may set
`minimum_items`. An option without a default must use `required=True`.

Override values by exact rule code:
...
Fensu validates declarations, defaults, required values, override types, choices,
and bounds before rule selection or evaluation. A rule can read only declarations
it owns; there is no string-keyed or cross-rule option API.
...
Pass `rule_options` to evaluate one source case under different overrides. The
harness uses the production resolver, applies defaults, rejects invalid or missing
values, and converts list overrides to tuples.
...
File rules retain per-file cache subjects and execute independently. Project rules
use a separate project cache subject with identity `.`; they are never attached to
an arbitrary source file. A cached project rule may own diagnostics and exceptions
at any confined repository-relative path. Query-specific tree and graph evidence
is replayed before reuse: broad inventory queries invalidate broadly, while focused
path, position, node, or edge queries can survive unrelated edits. Cached and
uncached diagnostics are rendered identically.

Resolved current values and complete option schemas participate in cache identity.
Changing an override, default, type, choice, or bound invalidates affected cached
results. `ctx.option()` is therefore safe in a cacheable rule.
