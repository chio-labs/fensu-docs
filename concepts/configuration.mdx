...
Fensu discovers the complete core and custom catalogue before validating these
tables. Unknown rule codes, option-free rules, unknown names, wrong scalar or list
types, missing required options, and failed choices or bounds stop the run before
evaluation. Options on unselected rules are validated too, so dormant typos cannot
remain hidden.
