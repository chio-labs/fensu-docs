...
Use explicit targets for multiple projects or a TypeScript/Svelte analyzer:
...
include `test_layout`, `framework`, `ui_kit`, `shadcn`, `openapi`, and `generated`;
the normal policy, threshold, exception, evaluation, cache, and scope keys live
inside each target too. See [TypeScript and SvelteKit](/concepts/analyzers/typescript-sveltekit)
for detection, setup, and current boundaries.
...
| `FW` | Every rule applicable to a TypeScript or Svelte target. |
...
analyzers use the `FW` catalogue described in
[TypeScript and SvelteKit](/concepts/analyzers/typescript-sveltekit).

### The default

`select` defaults to the analyzer's complete native policy: `["FF"]` for Python
and `["FW"]` for TypeScript or Svelte. If you want all of Fensu's opinion, set
nothing: the default already does it.

Native packs are not part of that default. A pack must be named in `rule_packs`
before its selectors become available.
...
`rule_packs` activates Fensu-shipped Rust policies by registered name. The
currently shipped pack is `dagster`:
