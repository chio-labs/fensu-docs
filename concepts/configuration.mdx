...
| `select` | list of selectors | `["SF"]` | Blocking rules to enable. |
...
| --- | --- |
| `SF` | Every core rule. |
| `SFR` | One core family (here: roles). |
| `SFR3` | One hundreds bucket (here: the `SFR3xx` layout rules). |
| `SFR301` | Exactly one rule. |
| `X` | Every custom rule. |
| `XDB` | A custom namespace (every rule whose code starts with `XDB`). |
| `XDB001` | Exactly one custom rule. |

The core family letters are `SFL`, `SFR`, `SFS`, `SFN`, `SFH`, `SFT`, and `SFA`;
...
ignore = ["SFH007", "SFA103"]   # ...except these two
```
...
```toml
select = ["SFL", "SFR", "SFS", "SFN", "SFH", "SFT"]
warn = ["SFA"]
```

A rule cannot resolve into both `select` and `warn`, or both `warn` and `ignore`.
Fensu compares resolved rule codes, so `select = ["SF"]` with `warn = ["SFA"]` is
...
| --- | --- | --- |
| `no-return` | `SFN001` | Do not return a meaningful value. |
| `returns-bool` | `SFN002` | Declare `bool`, `TypeGuard`, or `TypeIs`. |
| `returns-value` | `SFN003` | Do not explicitly declare a no-value result such as `None`. |
| `returns-iterator` | `SFN004` | Contain an owned `yield` or declare an iterator or generator result. |
