---
title: Rule Families
description: "The seven core rule families, native pack namespaces, and stable rule-code structure Fensu ships."
---
...
each rule's code. Fensu ships seven core families, reserves `FP` for
[native rule packs](/concepts/rule-packs), and reserves `X` for
...
reused, and existing rules are never renumbered. Fensu-shipped native packs use
`FP` plus a pack namespace, such as `FPDG022` for Dagster. Retained core aliases
preserve the family and number after that namespace, such as `FPDGA001` for
`FFA001`. Custom rules must use the `X` namespace (`X` + optional uppercase letters
+ digits, like `XDB001`) and are rejected at load if they start with `FF` or `FP`.
See [Native rule packs](/concepts/rule-packs) for shipped namespaces and
[Custom rules](/concepts/custom-rules#codes-and-namespaces) for how teams carve
their own `X` sub-namespaces.
...
| `FFR307` | top-level-direct-modules | Runtime roots and top-level domains do not contain ad hoc direct modules; deterministic root [public facades](/concepts/architecture-model#root-public-facades) are exempt. |
...
| `FFR310` | public-facade-import-direction | Runtime code imports a facade's owning domain directly; facade imports are reserved for tests, tooling, and external consumers. |
...
| `FFR403` | no-reexport-shim | Internal modules do not exist only to re-export imports; deterministic root public facades are exempt. |
| `FFR406` | public-surface-shape | A root `__init__.py` contains only a docstring, imports, and at most one `__all__` declaration. |
...
`FFR307`, `FFR403`, and `FFR310` work together for a recognized Python root
facade. The first two permit the bounded facade shape; the third prevents product
runtime code from depending inward on that outward-facing compatibility surface.
See [Root public facades](/concepts/architecture-model#root-public-facades) for the
recognized shapes and import direction.
...
| `FFT002` | test-scope | Test scopes come from the configured [`test_scopes`](/concepts/configuration#test-scopes). |
