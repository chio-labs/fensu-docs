...
The same standard is why duplicated code is reported, not enforced. Fensu can
prove deterministically that two functions match. It cannot prove that they
should be one, so [`fensu dupes`](/concepts/duplicated-code) presents the copies
as evidence and never fails a build.
