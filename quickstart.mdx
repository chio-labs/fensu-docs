...
<Note>
  The PyPI distribution is named `fensu`; the command it installs is
  `fensu`. Install it into the same environment as the project you check, so it
  parses your code with the interpreter it runs under.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Rule families" icon="list-check" href="/concepts/rule-families">
    What each family (SFL, SFR, SFS, SFN, SFH, SFT, SFA) checks.
  </Card>
