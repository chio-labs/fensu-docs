...
For Python targets, an immutable architecture graph resolved from static imports
and authoritative discovered modules. Unresolved imports remain explicit and
dynamic imports are not represented.
...
`ctx.rust`, `ctx.project.tree`, `ctx.source`, and stable locations. `ctx.graph`,
`ctx.syntax`, and `ctx.relations` are unavailable because parser implementation
objects are not part of the Rust authoring contract.
