...
Repository rules use `ctx.targets` instead of an unqualified target context.
`all()` returns every configured `RuleTargetFacts` handle in target-name order;
`named(name)` returns one exact handle or `None`.

Each handle exposes its stable `Target` identity, target-local `tree` and `graph`,
and the typed `python`, `rust`, or `web` workspace facts owned by its analyzer.
`repository_path(file_or_path)` and `repository_location(location)` translate
target-local evidence before constructing a repository-relative diagnostic.
Wrong-analyzer access and access to an analyzer omitted from the active rule's
`analyzers` fail closed.

Repository contexts have no current file, source, `ctx.project`, or unqualified
analyzer zones. Their queries use a distinct repository requester for cache replay.
See [Repository rules](/concepts/repository-rules) for the complete execution and
cache model.
...
During a repository-rule invocation, `path_fault()` accepts a repository-relative
path and may omit it to report against the repository subject `"."`.
`fault_at()` accepts a repository-relative `SourceLocation`; translate a
target-local location with `target.repository_location(...)` first.
...
| `Repository` | Immutable identity of one configured multi-target repository. |
| `Target` | Stable name, analyzer, and repository-relative root for one target. |
| `RuleTargetFacts` | One named target's identity, tree, graph, typed facts, and path translators. |
| `RepositoryTargetFacts` | Deterministic `all()` and `named()` target lookup. |
...
**Python repository facts**

| Model | Describes |
| --- | --- |
| `PythonWorkspaceFacts` | Versioned broad and focused Python file queries for one named target. |
| `PythonFileFacts` | One Python file's identity, source, and Fensu-owned fact, text, syntax, and relation zones. |
